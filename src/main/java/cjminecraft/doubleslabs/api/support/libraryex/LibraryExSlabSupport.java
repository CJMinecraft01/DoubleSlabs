package cjminecraft.doubleslabs.api.support.libraryex;
...
@SlabSupportProvider(modid = "libraryex")
public class LibraryExSlabSupport<T extends Enum<T> & IStringSerializable> implements IHorizontalSlabSupport {

    private final Class<?> slab;
    private final PropertyEnum<T> slabTypeProperty;
    private final T[] slabTypes;

    public LibraryExSlabSupport() {
        Class<?> slab;
        PropertyEnum<T> slabTypeProperty;
        T[] slabTypes;
        try {
            slab = Class.forName("logictechcorp.libraryex.block.BlockModSlab");
            slabTypeProperty = (PropertyEnum<T>) slab.getField("TYPE").get(null);
            Class<?> slabType = Class.forName("logictechcorp.libraryex.block.BlockModSlab$SlabType");
