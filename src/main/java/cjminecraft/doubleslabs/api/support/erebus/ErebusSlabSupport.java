package cjminecraft.doubleslabs.api.support.erebus;
...
@SlabSupportProvider(modid = "erebus")
public class ErebusSlabSupport<T extends Enum<T> & IStringSerializable> implements IHorizontalSlabSupport {

    private final Class<?> slab;
    private final PropertyEnum<T> slabTypeProperty;
    private final T[] slabTypes;

    public ErebusSlabSupport() {
        Class<?> slab;
        PropertyEnum<T> slabTypeProperty;
        T[] slabTypes;
        try {
            slab = Class.forName("erebus.blocks.BlockSlabErebus");
            slabTypeProperty = (PropertyEnum<T>) slab.getField("HALF").get(null);
            Class<?> slabType = Class.forName("erebus.blocks.BlockSlabErebus$EnumBlockHalf");
